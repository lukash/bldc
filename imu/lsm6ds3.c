...
void lsm6ds3_init(i2c_bb_state *i2c_state,
	...
	if (is_trc) {
		...
		commands_printf("SET FILTER: %#02x", txb[1]);
		res = i2c_bb_tx_rx(m_i2c_bb, lsm6ds3_addr, txb, 2, rxb, 1);
		if (!res) {
...
static THD_FUNCTION(lsm6ds3_thread, arg) {
	...
	while (!chThdShouldTerminateX()) {
		...
		// Sleep one tick less so that we ensure the frequency is at least
		// the configured one or a bit higher
		systime_t remaining_sleep_time = start_time + interval - chVTGetSystemTimeX() - 1;
